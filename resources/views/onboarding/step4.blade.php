@extends('layouts.app')
@section('title', isset($is_edit) ? 'Location & Match Preferences' : 'Setup — Step 4 of 5')
...
<div class="container py-5" style="max-width:640px">
    ...
    <form method="POST" action="{{ isset($is_edit) ? route('preferences.update') : route('setup.store', ['step' => 4]) }}">
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-body p-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    @elseif(!$isPremium)
                        <span class="badge bg-light text-muted border small">{{ 2 - $locationUpdates }} update{{ 2 - $locationUpdates === 1 ? '' : 's' }} left</span>
                    @else
                ...
                <div class="alert alert-danger rounded-3 py-2 mb-3 small">
                    <i class="bi bi-lock-fill me-2"></i>{{ $message }}
                    <a href="{{ route('premium.show') }}" class="alert-link fw-semibold ms-1">Upgrade →</a>
                </div>
                @enderror

                {{-- Lock / update-count banners --}}
                @if($locationLocked)
                <div class="alert alert-warning rounded-3 d-flex align-items-center gap-3 mb-3">
                    ...
                    <div class="small">
                        ...
                        <a href="{{ route('premium.show') }}" class="fw-semibold text-warning-emphasis ms-1">⭐ Go Premium</a>
                ...
                <div class="alert alert-info rounded-3 py-2 small mb-3">
                    ...
                    <a href="{{ route('premium.show') }}" class="fw-semibold ms-1">⭐ Go Premium</a> for unlimited updates.
                </div>
                @endif
                    ...
                    <div class="col-12">
                        ...
                        <select name="country" id="onbCountry"
                            ...
                            @foreach(\App\Helpers\CountryHelper::list() as $cName => $cCode)
                                <option value="{{ $cName }}" {{ $onbCountry === $cName ? 'selected' : '' }}>{{ $cName }}</option>
                            @endforeach
                        </select>
                        @if($locationLocked)
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-body p-4">
                ...
                <div class="mb-4">
                    ...
                    @error('seeking_gender')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                </div>
        ...
        <div class="d-flex justify-content-between">
            ...
            @else
                <a href="{{ route('setup.step', 2) }}" class="btn btn-outline-secondary rounded-pill px-4">
                    <i class="bi bi-arrow-left me-1"></i> Back
                </a>
            @endif
...
(function () {
    ...
    async function loadStates(country, stateVal, prefVal) {
        try {
            var res    = await fetch('{{ route('api.states') }}?country=' + encodeURIComponent(country));
