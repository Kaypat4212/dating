...
<head>
    ...
    @endphp
    <title>Privacy Policy — {{ SS::get('site_name', config('app.name')) }}</title>
...
<nav class="navbar navbar-expand sticky-top shadow-sm" style="background:rgba(13,1,24,.92);backdrop-filter:blur(16px);border-bottom:1px solid rgba(255,255,255,.06)">
    <div class="container">
        ...
        <div class="d-flex gap-2">
            @auth
            <a href="{{ route('dashboard') }}" class="btn btn-sm btn-outline-light rounded-pill px-3"><i class="bi bi-house me-1"></i>Dashboard</a>
            @else
...
<div class="legal-hero">
    <div class="container text-center">
        ...
        <p style="color:rgba(255,255,255,.45);font-size:.9rem" class="mb-4">Last updated: <strong style="color:rgba(255,255,255,.7)">{{ \Carbon\Carbon::now()->format('F j, Y') }}</strong></p>
        <div class="mt-3">
...
<div class="container py-5">
    <div class="legal-body">
        ...
        <div class="highlight-box mb-4">
            <p>This Privacy Policy explains how <strong>{{ config('app.name') }}</strong> collects, uses, stores, and protects your personal data. We are committed to transparency and giving you meaningful control over your information.</p>
        </div>
        ...
        <div class="legal-section" id="how-we-use">
            ...
            <p>We use your data only for legitimate purposes necessary to operate and improve {{ config('app.name') }}:</p>
            <ul>
        ...
        <div class="legal-section" id="sharing">
            ...
            <ul>
                ...
                <li><strong style="color:rgba(255,255,255,.8)">Legal Requirements:</strong> When required by law, court order, or to protect the rights, property, or safety of {{ config('app.name') }}, our users, or the public.</li>
        ...
        <div class="legal-section" id="security">
            ...
            <p>No system is 100% secure. If you discover a security vulnerability, please responsibly disclose it to <a href="mailto:{{ $securityEmail }}" style="color:#9ab4f5">{{ $securityEmail }}</a> rather than publicly disclosing it.</p>
        </div>
        ...
        <div class="legal-section" id="your-rights">
            ...
            <p>To exercise any of these rights, email us at <a href="mailto:{{ $privacyEmail }}" style="color:#9ab4f5">{{ $privacyEmail }}</a>. We will respond within 30 days (GDPR) or the timeframe required by applicable law. We may need to verify your identity before processing your request.</p>
        </div>

        <div class="legal-section" id="children">
            <div class="section-badge"><i class="bi bi-shield-exclamation"></i> Section 7</div>
            <h2>Children's Privacy</h2>
            <p>{{ config('app.name') }} is strictly for adults aged 18 and over. We do not knowingly collect or process personal data from anyone under the age of 18.</p>
            <p>If you believe a minor has registered on our platform or provided us with personal data, please contact us immediately at <a href="mailto:{{ $safetyEmail }}" style="color:#9ab4f5">{{ $safetyEmail }}</a>. We will investigate and delete any such data promptly.</p>
        </div>

        <div class="legal-section" id="international">
            <div class="section-badge"><i class="bi bi-globe2"></i> Section 8</div>
            <h2>International Data Transfers</h2>
            <p>{{ config('app.name') }} operates globally. Your data may be processed in countries other than your own, including countries that may have different data protection standards.</p>
        ...
        <div class="legal-section" id="changes">
            ...
            <p>We encourage you to review this policy periodically. Continued use of {{ config('app.name') }} after the effective date of changes constitutes acceptance of the updated policy.</p>
        </div>
        ...
        <div class="legal-section" id="contact">
            ...
            <p>{{ config('app.name') }} is the data controller for the personal data described in this policy. For privacy-related queries:</p>
            <div class="row g-3 mt-1">
                <div class="col-12 col-sm-6">
                    <div class="p-3 rounded-3 text-center" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07)">
                        ...
                        <a href="mailto:{{ $privacyEmail }}" style="color:#9ab4f5;font-size:.85rem">{{ $privacyEmail }}</a>
                    </div>
                </div>
                <div class="col-12 col-sm-6">
                    <div class="p-3 rounded-3 text-center" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07)">
                        ...
                        <a href="mailto:{{ $supportEmail }}" style="color:#9ab4f5;font-size:.85rem">{{ $supportEmail }}</a>
                    </div>
                </div>
            </div>
            <p class="mt-3">Also see our <a href="{{ route('legal.terms') }}" style="color:#9ab4f5">Terms &amp; Conditions</a> for other legal matters.</p>
        </div>
